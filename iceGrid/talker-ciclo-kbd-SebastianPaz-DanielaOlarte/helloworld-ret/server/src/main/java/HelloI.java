...
public class HelloI implements Hello {
    ...
    public void integrate(String method, String f, double lowerBound, double upperBound, int numIntervals) {
        ...
        for (int i = 0; i < numSubIntervals; i++) {
            ...
            reply += taskI(method + ":" + f + ":" + String.valueOf(subLowerBound) + ":" + String.valueOf(subUpperBound)
                    + ":" + String.valueOf(numIntervals / numSubIntervals));
        }
    ...
    private synchronized void solution(double response) {
        ...
        if (lstSolution.size() == numSubIntervals) {
            printResults();
            lstSolution.clear();
        }
    }
    ...
    public void shutdownAllClients() {
        for (Map.Entry<Integer, WorkerPrx> entry : lstConnected.entrySet()) {
            int id = entry.getKey();
            WorkerPrx wk = entry.getValue();
            wk.getResponse("exit");
            try {
                lstConnected.remove(id);
                System.out.println("Servidor desconectado.");
            } catch (Exception e) {
                System.err.println("Error al desconectar el cliente");
                e.printStackTrace();
            }
        }
        lstConnected.clear();
